...
**All phases 01–11 are code-complete; shipped as `v0.1.0` and patched to `v0.1.1`.** The React dashboard (08) is built (`dashboard/`, Vite + React + TS, five pages) and passes `bun run build` (tsc + Vite) and `bun run lint`. The pre-release gate has run — no-GPU suite (33 tests + self-checks), dashboard lint/build, and GPU equivalence spot-checks (`paged_equiv` bit-exact, `batched_equiv` within tol) all pass (`docs/PRE_RELEASE.md`). The under-load demo video (`docs/assets/demo.mp4`) was intentionally waived and is the one deferred nice-to-have.
...
- **Release status:** `v0.1.1` is the latest release (2026-07-06) — a patch over `v0.1.0` (benchmark JSON output, dashboard charting polish, internal cleanup). Its dashboard lint/build and headline-consistency gates passed; the GPU numerical spot-checks (`paged_equiv`, `batched_equiv`) were not re-run for this cut and are pending on the RTX 5090 box (the diff touches `core/model_runner.py`). `v0.1.0` (2026-07-03) was the first stable GitHub release off `main`, superseding pre-release `v0.1.0-rc.1`, with GPU equivalence spot-checks passed (`paged_equiv` bit-exact, `batched_equiv` within tol, against the base 9B). The under-load demo capture remains intentionally waived. See `docs/PRE_RELEASE.md`.
