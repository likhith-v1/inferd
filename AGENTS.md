...
- **Engine + serving live:** spec-decode, paged cache, continuous batching, FastAPI/SSE serving (`serve/`), the headless harness, plots, and `bench/report.md`. Sampling (`temperature`/`top_p`) is now per-request, overridable on each `/generate` call and falling back to the server default (`DECISIONS.md` 2026-07-15).
...
- **Release status:** `v0.1.1` (2026-07-06) is the latest *tagged* release — a patch over `v0.1.0` (benchmark JSON output, dashboard charting polish, internal cleanup); its dashboard lint/build and headline-consistency gates passed, and the GPU numerical spot-checks (`paged_equiv`, `batched_equiv`) were pending on the RTX 5090 box for that cut. `v0.1.5` (per-request sampling) is staged in the working tree but **not tagged**: it was authored on a non-GPU macOS session where the `uv` environment cannot resolve at all (`causal-conv1d` is a CUDA/Linux-only wheel), so the no-GPU test gate has not been run — see `docs/PRE_RELEASE.md` for exactly what's outstanding before tagging. `v0.1.0` (2026-07-03) was the first stable GitHub release off `main`, superseding pre-release `v0.1.0-rc.1`, with GPU equivalence spot-checks passed (`paged_equiv` bit-exact, `batched_equiv` within tol, against the base 9B). The under-load demo capture remains intentionally waived.
...
- Read `plan.md` (the design) and the `plans/shipped/NN_*.md` file for the phase you're working on (scope, owned/avoided files, commands, exit criteria, open questions). Future work is scoped in `plans/future/NN_*.md`.
- Read `plans/shipped/00_MASTER_ORCHESTRATION.md` for cross-phase rules (execution/merge order, the shared-file contract, quality gates); `plans/future/00_FUTURE_ROADMAP.md` is the equivalent index for the follow-up cycle.
...
## Workflow
- **Benchmark harness before optimization** (`plans/shipped/02`) — no speedup claim without a reproducible baseline.
...
- **`core/model_runner.py` is the hot shared file** (phases 04/05/06/10). Extend it via new methods per the interface contract in `plans/shipped/00`; never rewrite another phase's method without updating that contract.
...
- Model choice per phase is **deferred, not static** — each `plans/shipped/NN` (and `plans/future/NN`) header is a recommendation; the body's Model Selection section is the real decision.

## Repo layout (✅ = present, ⏳ = pending)
- ✅ `finetune/` (QLoRA + eval + export) · `core/` (spec_decode, paged_cache, paged_attn, scheduler, model_runner, qwen35_patch) · `serve/` (FastAPI + SSE: app, engine, schemas) · `bench/` (harness, correctness, runners, results, `run_all.py`, `report.md`) · `scripts/` (`hero_fp8.py`, `smoke_load.py`) · `tests/` · `plans/` (`shipped/` = v1 phases 00–11, `future/` = post-v0.1.0 roadmap) · `docs/` (`ENVIRONMENT.md`, `demo.md`) · `DECISIONS.md` · `README.md`
